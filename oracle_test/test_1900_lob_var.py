...
class TestCase(test_env.BaseTestCase):
    ...
    def __perform_test(self, lob_type, input_type):
        long_string = ""
        db_type = getattr(yaspy, lob_type)
    ...
    def __test_lob_operations(self, lob_type):
        ...
        self.assertEqual(len(lob), 75000)
        self.assertEqual(lob, long_string)
    ...
    def __test_temporary_lob(self, lob_type):
        ...
        db_type = getattr(yaspy, lob_type)
        lob = self.cursor.var(db_type).setvalue(value)
        self.cursor.execute("""
    ...
    def __validate_query(self, rows, lob_type):
        ...
        for row in rows:
            ...
            if integer_value == 0:
                self.assertEqual(len(lob), 0)
                expected_value = ""
                if lob_type == "BLOB":
                    expected_value = expected_value.encode()
                self.assertEqual(lob, expected_value)
            else:
                ...
                self.assertEqual(len(lob), len(expected_value))
                self.assertEqual(lob, expected_value)
                if lob_type == "CLOB":
    ...
    def test_1912_multiple_fetch(self):
        ...
        self.assertEqual(len(rows[0][1]), 19)
    ...
    def test_1915_nclob_non_ascii_chars(self):
        ...
        self.cursor.execute("select NCLOBCol from TestNCLOBs")
        nclob, = self.cursor.fetchone()
        self.assertEqual(nclob, value + value)
    ...
    def test_1919_assign_string_beyond_array_size(self):
        ...
        nclobVar = self.cursor.var(yaspy.NCLOB)
        self.assertRaises(IndexError, nclobVar.setvalue, "test char", 1)
