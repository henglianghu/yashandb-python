 ...
 * called by the setup_test.sql file after acquiring the necessary parameters
 * and also by the Python script setup_test.py.
 *---------------------------------------------------------------------------*/
...
create type &main_user..udt_Object as object (
    ...
    RealValue                           real,
    DoublePrecisionValue                double,
    FloatValue                          float,
    BinaryFloatValue                    float,
    BinaryDoubleValue                   double,
    DateValue                           date,
...
create table regress.TestTempTable (
    IntCol                              number(9) not null,
...
begin
    for i in 1..100 loop
        ...
        values (i, xmltype('<?xml version="1.0"?><records>' || dbms_random.string('x', 1024) || '</records>'));
    end loop;
end;
/
