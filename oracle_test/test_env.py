...
DEFAULT_CONNECT_STRING = "127.0.0.1:1688"
...
def get_client_version():
    ...
    value = PARAMETERS.get(name)
    if value is None:
        value = yaspy.clientversion()[:2]
        PARAMETERS[name] = value
    return value

def get_connection(**kwargs):
    return yaspy.connect(dsn=get_connect_string(), user=get_main_user(),
...
def get_pool(user=None, password=None, **kwargs):
    ...
    return yaspy.SessionPool(user, password, get_connect_string(),
                                **kwargs)
...
class RoundTripInfo:
    ...
    def __init__(self, connection):
        ...
        self.admin_conn = yaspy.connect(get_admin_connect_string())
    ...
    def get_db_object_as_plain_object(self, obj):
        if obj.type.iscollection:
            ...
            for value in obj.aslist():
                if isinstance(value, yaspy.Object):
                    value = self.get_db_object_as_plain_object(value)
                elif isinstance(value, yaspy.LOB):
                    value = value.read()
