...
class TestCase(test_env.BaseTestCase):
    ...
    def test_1206_execute_multiple_arg_types(self):
        ...
        simple_var = self.cursor.var(yaspy.NUMBER)
        dict_arg = dict(value=simple_var)
        self.assertRaises(yaspy.InterfaceError, self.cursor.execute,
    ...
    def test_1208_callproc(self):
        ...
        var = self.cursor.var(yaspy.NUMBER)
        self.assertRaises(yaspy.DatabaseError, self.cursor.callproc, "proc_Test", ("hi", 5, var))

    @unittest.skip
    def test_1209_callproc_all_keywords(self):
        "1209 - test executing a stored procedure with all args keyword args"
        inout_value = self.cursor.var(yaspy.NUMBER)
        inout_value.setvalue(0, 5)
        out_value = self.cursor.var(yaspy.NUMBER)
    ...
    def test_1211_callproc_repeated_keyword_parameters(self):
        ...
        kwargs = dict(a_InValue="hi",
                      a_OutValue=self.cursor.var(yaspy.NUMBER))
        self.assertRaises(yaspy.DatabaseError, self.cursor.callproc,
    ...
    def test_1213_callfunc(self):
        ...
        results = self.cursor.callfunc("func_Test", yaspy.NUMBER, ("hi", 5))
        self.assertEqual(results, 7)

    @unittest.skip
    def test_1214_callfunc_no_args(self):
        "1214 - test executing a stored function without any arguments"
        results = self.cursor.callfunc("func_TestNoArgs", yaspy.NUMBER)
    ...
    def test_1215_callfunc_negative(self):
        ...
        func_name = "func_Test"
        self.assertRaises(TypeError, self.cursor.callfunc, yaspy.NUMBER,
                          func_name, ("hi", 5))
        self.assertRaises(yaspy.DatabaseError, self.cursor.callfunc,
                          func_name, yaspy.NUMBER, ("hi", 5, 7))
        self.assertRaises(TypeError, self.cursor.callfunc, func_name,
                          yaspy.NUMBER, "hi", 7)
        self.assertRaises(yaspy.DatabaseError, self.cursor.callfunc,
                          func_name, yaspy.NUMBER, [5, "hi"])
        self.assertRaises(yaspy.DatabaseError, self.cursor.callfunc,
                          func_name, yaspy.NUMBER)
        self.assertRaises(TypeError, self.cursor.callfunc, func_name,
                          yaspy.NUMBER, 5)
    ...
    def test_1220_executemany_with_input_sizes_wrong(self):
        ...
        cursor = self.connection.cursor()
        cursor.setinputsizes(yaspy.NUMBER)
    ...
    def test_1221_executemany_with_multiple_batches(self):
        ...
        self.cursor.executemany(sql, [(3, "Testing"), (4, "Testing")])

    def test_1222_executemany_numeric(self):
        "1222 - test executemany() with various numeric types"
        self.cursor.execute("truncate table TestTempTable")
        data = [
            (decimal.Decimal("1"), decimal.Decimal("5")),
            (decimal.Decimal("2"), decimal.Decimal("7.0")),
            (decimal.Decimal("3"), decimal.Decimal("6.5")),
            (decimal.Decimal("4"), decimal.Decimal(2 ** 65)),
            (decimal.Decimal("5"), decimal.Decimal("24.5"))
        ]
    ...
    def test_1224_executemany_with_exception(self):
        ...
        statement = "insert into TestTempTable (IntCol) values (:value)"
        self.assertRaises(yaspy.DatabaseError, self.cursor.executemany,
                          statement, rows)
    ...
    def test_1225_executemany_with_invalid_parameters(self):
        ...
        sql = "insert into TestTempTable (IntCol, StringCol) values (:1, :2)"
        self.assertRaises(yaspy.ProgrammingError, self.cursor.executemany, sql,
    ...
    def test_1231_iterators_interrupted(self):
        ...
        self.assertRaises(yaspy.InterfaceError, next, test_iter)

    @unittest.skip
    def test_1232_bind_names(self):
        "1232 - test that bindnames() works correctly."
        self.assertRaises(yaspy.ProgrammingError, self.cursor.bindnames)
    ...
    def test_1233_bad_execute(self):
        ...
        self.assertRaises(yaspy.DatabaseError,
                          self.cursor.execute,
    ...
    def test_1234_fetch_after_bad_execute(self):
        ...
        self.assertRaises(yaspy.InterfaceError, self.cursor.fetchall)
    ...
    def test_1235_scroll_absolute_exception_after(self):
        ...
        cursor.execute("""
                ...
                from TestNumbers
                order by IntCol""")
        self.assertRaises(yaspy.DatabaseError, cursor.scroll, 12,
                          "absolute")
    ...
    def test_1241_scroll_relative_exception_after(self):
        ...
        cursor.execute("""
                ...
                from TestNumbers
                order by IntCol""")
        self.assertRaises(yaspy.DatabaseError, cursor.scroll, 15)
    ...
    def test_1242_scroll_relative_exception_before(self):
        ...
        cursor.execute("""
                ...
                from TestNumbers
                order by IntCol""")
        self.assertRaises(yaspy.DatabaseError, cursor.scroll, -5)
    ...
    def test_1245_scroll_no_rows(self):
        ...
        cursor.scroll(mode="first")
        self.assertEqual(cursor.fetchall(), [])
        self.assertRaises(yaspy.DatabaseError, cursor.scroll, 1,
                          mode="absolute")
    ...
    def test_1247_set_input_sizes_negative(self):
        ...
        self.assertRaises(yaspy.InterfaceError,
    ...
    def test_1251_set_input_sizes_by_position(self):
        ...
        var = self.cursor.var(yaspy.VARCHAR, 100)
        self.cursor.setinputsizes(None, 5, None, 10, None, yaspy.NUMBER)
        self.cursor.execute("""
                begin
    ...
    def test_1254_parse_query(self):
        ...
        self.assertEqual(self.cursor.description,
                         [('LONGINTCOL', yaspy.NUMBER, 17, None,
                           16, 0, 0)])
    ...
    def test_1259_as_context_manager(self):
        ...
        self.assertRaises(yaspy.InterfaceError, self.cursor.close)
    ...
    def test_1270_execute_bind_names_with_incorrect_bind(self):
        ...
        self.assertRaises(yaspy.DatabaseError, self.cursor.execute,
                          statement, value2=3)
    ...
    def test_1274_execute_with_rebinding_bind_name(self):
        ...
        simple_var = self.cursor.var(yaspy.NUMBER)
        simple_var2 = self.cursor.var(yaspy.NUMBER)
        simple_var2.setvalue(5)
        result = self.cursor.execute(statement, value=simple_var,
                                     value2=simple_var2)
        self.assertEqual(result, None)
        self.assertEqual(simple_var.getvalue(), '10')

        simple_var = self.cursor.var(yaspy.FLOAT)
        simple_var2 = self.cursor.var(yaspy.DOUBLE)
        simple_var2.setvalue(10.0)
        result = self.cursor.execute(statement, value=simple_var,
                                     value2=simple_var2)
        self.assertEqual(result, None)
        self.assertEqual(simple_var.getvalue(), 15.0)
    ...
    def test_1276_bind_by_name_with_duplicates(self):
        ...
        statement = "begin :value := :value + 5; end;"
        simple_var = self.cursor.var(yaspy.NUMBER)
        simple_var.setvalue(5)
        self.assertRaises(yaspy.DatabaseError, self.cursor.execute, statement, value=simple_var)
    ...
    def test_1277_positional_bind_with_duplicates(self):
        ...
        simple_var = self.cursor.var(yaspy.NUMBER)
        simple_var.setvalue(5)
        self.assertRaises(yaspy.DatabaseError, self.cursor.execute, statement, [simple_var])
    ...
    def test_1286_encodingErrors_deprecation(self):
        ...
        errors = 'strict'
        self.assertRaises(yaspy.ProgrammingError, self.cursor.var,
                          yaspy.NUMBER, encoding_errors=errors,
    ...
    def test_1287_keywordParameters_deprecation(self):
        ...
        out_value = self.cursor.var(yaspy.NUMBER)
        kwargs = dict(a_OutValue=out_value)
        self.assertRaises(yaspy.ProgrammingError, self.cursor.callproc,
                          "proc_Test", ("hi", 5), kwargs,
                          keywordParameters=kwargs)
        extra_amount = self.cursor.var(yaspy.NUMBER)
        extra_amount.setvalue(0, 5)
        kwargs = dict(a_ExtraAmount=extra_amount, a_String="hi")
        self.assertRaises(yaspy.ProgrammingError, self.cursor.callfunc,
                          "func_Test", yaspy.NUMBER, [], kwargs,
                          keywordParameters=kwargs)
