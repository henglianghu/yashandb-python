...
class TestCase(test_env.BaseTestCase):
    ...
    def setUp(self):
        ...
        for i in range(1, 11):
            ...
            date_col = yaspy.DateFromTicks(int(time_in_ticks))
            if i % 2:
                time_in_ticks = time.mktime(time_tuple) + i * 86400 * 2 + \
                        i * 12960
                nullable_col = yaspy.DateFromTicks(int(time_in_ticks))
            else:
                nullable_col = None
            tuple = (decimal.Decimal(i), date_col, nullable_col)
    ...
    def test_1400_bind_date(self):
        ...
        self.cursor.execute("""
                ...
                where DateCol = :value""",
                value = yaspy.Timestamp(2002, 12, 13, 9, 36, 0))
    ...
    def test_1402_bind_date_in_datetime_var(self):
        ...
        var = self.cursor.var(yaspy.DATETIME)
        date_val = datetime.date.today()
        var.setvalue(date_val)
        self.cursor.execute("select DATE(:1, 'YYYY-MM-DD') from dual", [var])
        result, = self.cursor.fetchone()
        self.assertEqual(result, date_val)
    ...
    def test_1405_bind_date_array_direct(self):
        ...
        return_value = self.cursor.var(yaspy.NUMBER)
        ...
        self.cursor.execute(statement, return_value=return_value,
                            start_value=5,
                            base_date=yaspy.Date(2002, 12, 12), array=array)
        self.assertEqual(return_value.getvalue(), 35.5)
        array = array + array[:5]
        self.cursor.execute(statement, start_value=7,
                            base_date=yaspy.Date(2002, 12, 13), array=array)
    ...
    def test_1406_bind_date_array_by_sizes(self):
        ...
        return_value = self.cursor.var(yaspy.NUMBER)
        self.cursor.setinputsizes(array=[yaspy.DATETIME, 10])
        array = [r[1] for r in self.raw_data]
        self.cursor.execute("""
                begin
                  :return_value := pkg_TestDateArrays.TestInArrays(
                ...
                start_value=6,
                base_date=yaspy.Date(2002, 12, 13),
                array=array)
    ...
    def test_1407_bind_date_array_by_var(self):
        ...
        return_value = self.cursor.var(yaspy.NUMBER)
        array = self.cursor.arrayvar(yaspy.DATETIME, 10, 20)
        ...
        self.cursor.execute("""
                ...
                return_value=return_value,
                start_value=7,
                base_date=yaspy.Date(2002, 12, 14),
                array=array)
    ...
    def test_1408_bind_in_out_date_array_by_var(self):
        ...
        array = self.cursor.arrayvar(yaspy.DATETIME, 10, 100)
        ...
        self.assertEqual(array.getvalue(),
                [ yaspy.Timestamp(2002, 12, 17, 2, 24, 0),
                  yaspy.Timestamp(2002, 12, 18, 4, 48, 0),
                  yaspy.Timestamp(2002, 12, 19, 7, 12, 0),
                  yaspy.Timestamp(2002, 12, 20, 9, 36, 0),
                  yaspy.Timestamp(2002, 12, 21, 12, 0, 0) ] + \
                original_data[5:])

    @unittest.skip
    def test_1409_bind_out_date_array_by_var(self):
        "1409 - test binding out a date array (with arrayvar)"
        array = self.cursor.arrayvar(yaspy.DATETIME, 6, 100)
        self.cursor.execute("""
                begin
                ...
                array=array)
        self.assertEqual(array.getvalue(),
                [ yaspy.Timestamp(2002, 12, 13, 4, 48, 0),
                  yaspy.Timestamp(2002, 12, 14, 9, 36, 0),
                  yaspy.Timestamp(2002, 12, 15, 14, 24, 0),
                  yaspy.Timestamp(2002, 12, 16, 19, 12, 0),
                  yaspy.Timestamp(2002, 12, 18, 0, 0, 0),
                  yaspy.Timestamp(2002, 12, 19, 4, 48, 0) ])

    @unittest.skip
    def test_1410_bind_out_set_input_sizes(self):
        "1410 - test binding out with set input sizes defined"
        bind_vars = self.cursor.setinputsizes(value=yaspy.DATETIME)
        self.cursor.execute("""
                begin
                  :value := to_date(20021209, 'YYYYMMDD');
                end;""")
        self.assertEqual(bind_vars["value"].getvalue(),
               yaspy.Timestamp(2002, 12, 9))
    ...
    def test_1411_bind_in_out_set_input_sizes(self):
        ...
        self.cursor.execute("""
                ...
                value=yaspy.Timestamp(2002, 12, 12, 10, 0, 0))
        self.assertEqual(bind_vars["value"].getvalue(),
                yaspy.Timestamp(2002, 12, 17, 16, 0, 0))
    ...
    def test_1414_cursor_description(self):
        ...
        expected_value = [
            ('INTCOL', 12, 20, 6, 9, 0, 0),
            ('DATECOL', 13, 32, 8, None, None, 0),
            ('NULLABLECOL', 13, 32, 8, None, None, 1)
        ]
