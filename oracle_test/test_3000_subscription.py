...
class SubscriptionData:
    ...
    def callback_handler(self, message):
        if message.type != yaspy.EVENT_DEREG:
            self._process_message(message)
            self.num_messages_received += 1
        if message.type == yaspy.EVENT_DEREG or \
...
class TestCase(test_env.BaseTestCase):
    ...
    def test_3000_dml_subscription(self):
        ...
        table_operations = [
            ...
            yaspy.OPCODE_INSERT,
            yaspy.OPCODE_DELETE,
            yaspy.OPCODE_ALTER | yaspy.OPCODE_ALLROWS
        ]
        row_operations = [
            yaspy.OPCODE_INSERT,
            yaspy.OPCODE_UPDATE,
        ...
        sub = connection.subscribe(callback=data.callback_handler,
                                   timeout=10, qos=yaspy.SUBSCR_QOS_ROWIDS)
    ...
    def test_3001_deprecations(self):
        ...
        connection = test_env.get_connection(threaded=True, events=True)
        self.assertRaises(yaspy.ProgrammingError, connection.subscribe,
    ...
    def test_3002_aq_subscription(self):
        ...
        sub = connection.subscribe(namespace=yaspy.SUBSCR_NAMESPACE_AQ,
