...
class TestCase(test_env.BaseTestCase):
    ...
    def setUp(self):
        ...
        for i in range(1, 11):
            ...
            date_value = yaspy.TimestampFromTicks(int(time_in_ticks))
            str_value = str(i * 50)
            fsecond = int(str_value + "0" * (6 - len(str_value)))
            date_col = yaspy.Timestamp(date_value.year, date_value.month,
            ...
            if i % 2:
                ...
                str_value = str(i * 125)
                fsecond = int(str_value + "0" * (6 - len(str_value)))
                nullable_col = yaspy.Timestamp(date_value.year,
                                                 date_value.month,
                                                 date_value.day,
                                                 date_value.hour,
    ...
    def test_2600_bind_timestamp(self):
        ...
        # self.cursor.setinputsizes(value=yaspy.DB_TYPE_TIMESTAMP)
        self.cursor.execute("""
                select * from TestTimestamps
                where TimestampCol = :value""",
                value=yaspy.Timestamp(2002, 12, 14, 0, 0, 10, 250000))
    ...
    def test_2602_bind_out_set_input_sizes(self):
        ...
        bind_vars = self.cursor.setinputsizes(value=yaspy.DB_TYPE_TIMESTAMP)
        self.cursor.execute("""
                begin
    ...
    def test_2605_bind_in_out_var_direct_set(self):
        ...
        var1 = self.cursor.var(yaspy.DATETIME)
        var2 = self.cursor.var(yaspy.DATETIME)
        var2.setvalue(yaspy.Timestamp(2002, 12, 9, 6, 0, 0))
        self.cursor.execute("""
                begin
                  :value1 := :value2 + 5.25;
                end;""",
                value1 = var1, value2=var2)
        self.assertEqual(var1.getvalue(),
                         yaspy.Timestamp(2002, 12, 14, 12, 0, 0))
    ...
    def test_2606_cursor_description(self):
        ...
        expected_value = [
            ...
            ('TIMESTAMPCOL', 16, 64, 8, 6, None, 0),
            ('NULLABLECOL', 16, 64, 8, 6, None, 1)
        ]
